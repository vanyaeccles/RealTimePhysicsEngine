...
glm::vec3 velVecY(0.0f, 0.0f, 0.0f);
...
#pragma region "Shaders"
	...
	Shader skyboxShader("U:/Physics/Phys4/Stuff/Shaders/skybox/skybox.vert", "U:/Physics/Phys4/Stuff/Shaders/skybox/skybox.frag");
	
	Shader whiteShader("U:/Physics/Phys4/Stuff/Shaders/plaincolour/white.vert", "U:/Physics/Phys4/Shaders/plaincolour/white.frag");
	...
	Shader boundboxShader("U:/Physics/Phys4/Stuff/Shaders/plaincolour/boundbox.vert", "U:/Physics/Phys4/Stuff/Shaders/plaincolour/boundbox.frag");
...
#pragma region "Models and Textures"
	...
	// Load models
	Model cube("U:/Physics/Phys4/Stuff/Models/cube.obj");
	Model modelCopyMeshes("U:/Physics/Phys4/Stuff/Models/cube.obj");

	Model sphere("U:/Physics/Phys4/Stuff/Models/sphere/sphere.obj");
...
#pragma endregion
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < bpcd.bodies.size(); i++)
		{
			...
			model = glm::mat4();
			model = glm::translate(model, bpcd.bodies[i].position);
			...
			if ((boundB || bruteFB) && !boundS)
			{
				...
				//model = glm::mat4();
				model = glm::translate(model, bpcd.bodies[i].position);
				model = glm::scale(model, glm::vec3(bpcd.bodies[i].xDist, bpcd.bodies[i].yDist, bpcd.bodies[i].zDist));
				glUniformMatrix4fv(glGetUniformLocation(boundboxShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
				glUniform4f(glGetUniformLocation(boundboxShader.Program, "boundBoxColour"), bpcd.bodies[i].boundBoxColour.x, bpcd.bodies[i].boundBoxColour.y, bpcd.bodies[i].boundBoxColour.z, 1.0f);
			...
			if (boundS)
			{
				...
				//model = glm::mat4();
				model = glm::translate(model, bpcd.bodies[i].position);
				model = glm::scale(model, glm::vec3(bpcd.bodies[i].boundSphere.radius * 66.0f, bpcd.bodies[i].boundSphere.radius * 66.0f, bpcd.bodies[i].boundSphere.radius * 66.0f));
				glUniformMatrix4fv(glGetUniformLocation(boundboxShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
				glUniform4f(glGetUniformLocation(boundboxShader.Program, "boundBoxColour"), bpcd.bodies[i].boundBoxColour.x, bpcd.bodies[i].boundBoxColour.y, bpcd.bodies[i].boundBoxColour.z, 1.0f);
				sphere.Draw(boundboxShader);
			}
		...
		if (playSimulation)
		{
			...
			for (int i = 0; i < bpcd.bodies.size(); i++)
			{
				...
				bpcd.bodies[0].angVelocity = angVec;

				bpcd.bodies[0].velocity = velVecX + velVecY + velVecZ;
			}
		}
		...
		if (boundS) // Press R
		{
			bpcd.checkBoundSphereCollision();
		}

		if (bruteFB) // Press Y
		{
			bpcd.checkBoundBoxBruteForceCollision();
		}

		if (boundB) // Press T
		{
			bpcd.checkBoundBoxCollision();
		}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (key == GLFW_KEY_L && action == GLFW_PRESS)
		velVecX += glm::vec3(0.01f, 0.0f, 0.0f);
	if (key == GLFW_KEY_K && action == GLFW_PRESS)
		velVecX -= glm::vec3(0.01f, 0.0f, 0.0f);

	if (key == GLFW_KEY_P && action == GLFW_PRESS)
		velVecY += glm::vec3(0.0f, 0.01f, 0.0f);
	if (key == GLFW_KEY_O && action == GLFW_PRESS)
		velVecY -= glm::vec3(0.0f, 0.01f, 0.0f);

	if (key == GLFW_KEY_J && action == GLFW_PRESS)
		velVecZ += glm::vec3(0.0f, 0.0f, 0.01f);
