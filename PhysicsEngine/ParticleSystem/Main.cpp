// Basic OpenGL code adapted from the online tuturials by Joey de Vries found at 'http://learnopengl.com/'
...
glm::vec3 fan(0.0f, 0.0f, 5.0f);


glm::vec3 plane1Normal(0.0, 1.0, 0.0);
glm::vec3 plane1Point(0.0f, -6.0f, -4.0f);
...
#pragma region "Shaders"
	...
	Shader whiteShader("U:/Physics/VanyaOpenGL/Stuff/Shaders/plaincolour/white.vert", "U:/Physics/VanyaOpenGL/Stuff/Shaders/plaincolour/white.frag");

	Shader blueShader("U:/Physics/VanyaOpenGL/Stuff/Shaders/plaincolour/blue.vert", "U:/Physics/VanyaOpenGL/Stuff/Shaders/plaincolour/blue.frag");
...
#pragma region "Models and Textures"
	...
	// Load models
	Model sphere("U:/Physics/VanyaOpenGL/Stuff/Models/sphere/sphere.obj");
...
#pragma endregion
	...
	// Initialise particle system with particles in various positions within a 10.0f by 10.0f area
	ParticleSystemSim partisys;
	
	partisys.initializeParticles();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Draw scene, create camera transformations
		glm::mat4 model;
		glm::mat4 view = camera.GetViewMatrix();
		glm::mat4 projection = glm::perspective(camera.Zoom, (float)screenWidth / (float)screenHeight, 0.1f, 100.0f);
		glm::vec3 velocityColour;
		...
		{
			float partX = partisys.parti[i].position[0];
			float partY = partisys.parti[i].position[1];
			float partZ = partisys.parti[i].position[2];

			GLfloat rColour = partisys.parti[i].colour[0];
			GLfloat gColour = partisys.parti[i].colour[1];
			GLfloat bColour = partisys.parti[i].colour[2];
			...
			glm::vec3 velocityColour = glm::vec3(rColour, gColour, bColour);
			GLint colourLocation = glGetUniformLocation(whiteShader.Program, "velocityColour");
			glUniform4f(colourLocation, rColour, 0.0f, 0.0f, 0.0f);
		...
		if (playSimulation)
		{
			...
			partisys.update(timestep, reverseGrav, rightFan, leftFan);

			partisys.particlePlaneCollisionCheck(plane1Normal, plane1Point);
		}
		...
		//Renders a quad in wireframe mode
		//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
		blueShader.Use();
		glUniform3f(glGetUniformLocation(blueShader.Program, "cameraPos"), camera.Position.x, camera.Position.y, camera.Position.z);
		glUniformMatrix4fv(glGetUniformLocation(blueShader.Program, "view"), 1, GL_FALSE, glm::value_ptr(view));
		glUniformMatrix4fv(glGetUniformLocation(blueShader.Program, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
		model = glm::mat4();
		model = glm::translate(model, glm::vec3(10.0f, -6.06f, 5.0f));
		model = glm::scale(model, glm::vec3(10.0f, 15.0f, 6.0f));
		model = glm::rotate(model, ninety, glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(glGetUniformLocation(blueShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
	    RenderQuad();
